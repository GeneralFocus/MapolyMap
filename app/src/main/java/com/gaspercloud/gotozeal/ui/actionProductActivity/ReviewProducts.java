...
import com.gaspercloud.gotozeal.adapter.List_Order_Adapter_Review;
...
public class ReviewProducts extends AppCompatActivity {
    ...
    private List_Order_Adapter_Review order_adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean _status = getIntent().getBooleanExtra("status", false);
        orderModelList = new ArrayList<>();
        order_adapter = new List_Order_Adapter_Review(context, orderModelList, this, _status);
        ...
        //prepareOrderData(productModel.getId(), context, null, null, _status);
        prepareOrderData(prdID, context, null, null, _status);
        //initScrollListener();
    }
    ...
    private void prepareOrderData(final Integer customer, Context context,
        ...
        homeViewModel.listOrders(customer, context, per_page).observe(this, new Observer<List<OrderModel>>() {
            ...
            public void onChanged(List<OrderModel> ordModels) {
                ...
                for (OrderModel model : ordModels) {
                    Log.i("FIND_ABU", model.toString());
                    if (customer == 0) {
                        orderModelList.add(model);
                    } else if (model.getParent_id() == customer && model.getCustomer_note() != null && !model.getCustomer_note().isEmpty()) {
                        orderModelList.add(model);
                    }
